...
try {
  const r = await c.query('SELECT insert_image_embedding($1, $2, $3, $4, $5) AS id', [t, 'http://x.com/t4.jpg', 'h-' + Date.now(), '[' + v + ']', '{}']);
  console.log('inserted:', r.rows);
  await c.query("DELETE FROM public.image_embeddings WHERE image_url = $1", ['http://x.com/t4.jpg']);
} catch (e) {
